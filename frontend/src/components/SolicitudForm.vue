...
<template>
  <form class="form card" @submit.prevent="submit">
    <div class="field"><label>Título</label><input v-model="form.titulo" data-testid="form-titulo" maxlength="120"><small v-if="errores.titulo" class="error" data-testid="error-titulo">{{ errores.titulo }}</small></div>
    <div class="field"><label>Descripción</label><textarea v-model="form.descripcion" data-testid="form-descripcion" rows="7" maxlength="4000" /><small v-if="errores.descripcion" class="error" data-testid="error-descripcion">{{ errores.descripcion }}</small></div>
    <div class="form__grid"><div class="field"><label>Categoría</label><select v-model="form.categoriaId" data-testid="form-categoria"><option value="">Selecciona…</option><option v-for="c in categorias" :key="c.id" :value="c.id">{{ c.nombre }} · {{ c.slaHoras }} h</option></select><small v-if="errores.categoria" class="error" data-testid="error-categoria">{{ errores.categoria }}</small></div>
    <div class="field"><label>Prioridad</label><select v-model="form.prioridad" data-testid="form-prioridad"><option v-for="p in prioridades" :key="p">{{ p }}</option></select></div></div>
